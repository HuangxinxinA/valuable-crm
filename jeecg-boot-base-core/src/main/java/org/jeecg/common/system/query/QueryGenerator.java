...
public class QueryGenerator {
     ...
     * @return QueryWrapper实例
     */
    public static <T> MPJQueryWrapper<T> initMPJQueryWrapper(T searchObj, Map<String, String[]> parameterMap, String alias) {
        long start = System.currentTimeMillis();
        MPJQueryWrapper<T> queryWrapper = new MPJQueryWrapper<T>();
        installMPJMplus(queryWrapper, searchObj, parameterMap, alias);
        log.debug("---查询条件构造器初始化完成,耗时:" + (System.currentTimeMillis() - start) + "毫秒----");
        return queryWrapper;
    }
    ...
    private static void installMPJMplus(MPJQueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap, String alias) {
		...
		*/
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            try {
                ...
                //数据权限查询
                if (ruleMap.containsKey(name)) {
                    addRuleToMPJQueryWrapper(ruleMap.get(name), column, origDescriptors[i].getPropertyType(), queryWrapper, alias);
                }
                //区间查询
                doIntervalQueryMPJ(queryWrapper, parameterMap, type, name, column, alias);
                //判断单值  参数带不同标识字符串 走不同的查询
                //TODO 这种前后带逗号的支持分割后模糊查询(多选字段查询生效) 示例：,1,3,
                if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
                    String multiLikeval = value.toString().replace(",,", COMMA);
                    ...
                    //}
                    // add -end 添加判断为字符串时设为全模糊查询
                    addEasyQueryMPJ(queryWrapper, column, rule, value, alias);
                }
        ...
        // 排序逻辑 处理
        doMultiFieldsOrderMPJ(queryWrapper, parameterMap, fieldColumnMap);

        //高级查询
        doSuperQueryMPJ(queryWrapper, parameterMap, fieldColumnMap, alias);
        // update-end--Author:taoyan  Date:20200923 for：issues/1671 如果字段加注解了@TableField(exist = false),不走DB查询-------
    ...
    private static void doIntervalQueryMPJ(MPJQueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap, String type, String filedName, String columnName, String alias) throws ParseException {
        // 添加 判断是否有区间值
        String endValue = null, beginValue = null;
        if (parameterMap != null && parameterMap.containsKey(filedName + BEGIN)) {
            beginValue = parameterMap.get(filedName + BEGIN)[0].trim();
            addQueryByRuleMPJ(queryWrapper, columnName, type, beginValue, QueryRuleEnum.GE, alias);

        }
        if (parameterMap != null && parameterMap.containsKey(filedName + END)) {
            endValue = parameterMap.get(filedName + END)[0].trim();
            addQueryByRuleMPJ(queryWrapper, columnName, type, endValue, QueryRuleEnum.LE, alias);
        }
        //多值查询
        if (parameterMap != null && parameterMap.containsKey(filedName + MULTI)) {
            endValue = parameterMap.get(filedName + MULTI)[0].trim();
            addQueryByRuleMPJ(queryWrapper, columnName.replace(MULTI, ""), type, endValue, QueryRuleEnum.IN, alias);
        }
    }
    ...
    private static void doMultiFieldsOrderMPJ(MPJQueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap, Map<String, String> fieldColumnMap) {
        Set<String> allFields = fieldColumnMap.keySet();
        String column = null, order = null;
        if (parameterMap != null && parameterMap.containsKey(ORDER_COLUMN)) {
            column = parameterMap.get(ORDER_COLUMN)[0];
        ...
        if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
            ...
            // 修改为 _ 前端传递排序条件{....,column: 'column1,column2',order: 'desc'} 翻译成sql "column1 desc,column2 desc"
            //拼接别名
            List<String> sqlInjectSortFields = (List<String>) SqlInjectionUtil.getSqlInjectSortFields(column.split(","))
                    .stream().map(item -> queryWrapper.getAlias() + "." + item).collect(Collectors.toList());
            if (order.toUpperCase().indexOf(ORDER_TYPE_ASC) >= 0) {
                queryWrapper.orderByAsc(sqlInjectSortFields);
            } else {
                queryWrapper.orderByDesc(sqlInjectSortFields);
            }
            //update-end--Author:scott  Date:20210531 for：36 多条件排序无效问题修正-------
        }
    }
    ...
    private static void doSuperQueryMPJ(MPJQueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap, Map<String, String> fieldColumnMap, String alias) {
        if (parameterMap != null && parameterMap.containsKey(SUPER_QUERY_PARAMS)) {
            String superQueryParams = parameterMap.get(SUPER_QUERY_PARAMS)[0];
            String superQueryMatchType = parameterMap.get(SUPER_QUERY_MATCH_TYPE) != null ? parameterMap.get(SUPER_QUERY_MATCH_TYPE)[0] : MatchTypeEnum.AND.getValue();
            MatchTypeEnum matchType = MatchTypeEnum.getByValue(superQueryMatchType);
                ...
                queryWrapper.and(andWrapper -> {
                    for (int i = 0; i < filterConditions.size(); i++) {
                        ...
                        if (oConvertUtils.isNotEmpty(rule.getField())
                            ...
                            // update-begin--author:sunjianlei date:20210702 for：【/issues/I3VR8E】高级查询没有类型转换，查询参数都是字符串类型 ----
                            addEasyQueryMPJ(andWrapper, fieldColumnMap.get(rule.getField()), QueryRuleEnum.getByValue(rule.getRule()), queryValue, alias);
                            //update-end-author:taoyan date:20201228 for: 【高级查询】 oracle 日期等于查询报错
    ...
    private static void addQueryByRuleMPJ(MPJQueryWrapper<?> queryWrapper, String name, String type, String value, QueryRuleEnum rule, String alias) throws ParseException {
        if (oConvertUtils.isNotEmpty(value)) {
            //update-begin--Author:sunjianlei  Date:20220104 for：【JTC-409】修复逗号分割情况下没有转换类型，导致类型严格的数据库查询报错 -------------------
            // 针对数字类型字段，多值查询
            if (value.contains(COMMA)) {
                ...
                addEasyQueryMPJ(queryWrapper, name, rule, temp, alias);
                return;
            }
            Object temp = QueryGenerator.parseByType(value, type, rule);
            addEasyQueryMPJ(queryWrapper, name, rule, temp, alias);
    ...
    public static void addEasyQueryMPJ(MPJQueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value, String alias) {
        if (name == null || value == null || rule == null || oConvertUtils.isEmpty(value)) {
            return;
        }
        name = (alias == null ? queryWrapper.getAlias() : alias) + "." + oConvertUtils.camelToUnderline(name);
        log.debug("---高级查询 Query规则---field:{} , rule:{} , value:{}", name, rule.getValue(), value);
        switch (rule) {
            case GT:
                queryWrapper.gt(name, value);
    ...
    private static void addRuleToMPJQueryWrapper(SysPermissionDataRuleModel dataRule, String name, Class propertyType, MPJQueryWrapper<?> queryWrapper, String alias) {
        QueryRuleEnum rule = QueryRuleEnum.getByValue(dataRule.getRuleConditions());
        if (rule.equals(QueryRuleEnum.IN) && !propertyType.equals(String.class)) {
            String[] values = dataRule.getRuleValue().split(",");
            Object[] objs = new Object[values.length];
            ...
            addEasyQueryMPJ(queryWrapper, name, rule, objs, alias);
        } else {
            if (propertyType.equals(String.class)) {
                addEasyQueryMPJ(queryWrapper, name, rule, converRuleValue(dataRule.getRuleValue()), alias);
            } else if (propertyType.equals(Date.class)) {
                String dateStr = converRuleValue(dataRule.getRuleValue());
                int length = 10;
                if (dateStr.length() == length) {
                    addEasyQueryMPJ(queryWrapper, name, rule, DateUtils.str2Date(dateStr, DateUtils.date_sdf.get()), alias);
                } else {
                    addEasyQueryMPJ(queryWrapper, name, rule, DateUtils.str2Date(dateStr, DateUtils.datetimeFormat.get()), alias);
                }
            } else {
                addEasyQueryMPJ(queryWrapper, name, rule, NumberUtils.parseNumber(dataRule.getRuleValue(), propertyType), alias);
            }
        }
    }
