...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
    ...
    @Override
    public String addSysCategory(SysCategory sysCategory) {
        String categoryCode = sysCategory.getCode();
        ...
        if (StringUtils.isNotBlank(categoryCode)) {
            SysCategory category = baseMapper.queryByCodeAndPid(sysCategory.getCode(), sysCategory.getPid());
            if (category != null && category.getPid().equals(sysCategory.getPid())) {
                return "编码已存在";
            }
        } else {
    ...
    public String updateSysCategory(SysCategory sysCategory) {
        ...
        if (StringUtils.isNotBlank(sysCategory.getCode())) {
            SysCategory category = baseMapper.queryByCodeAndPid(sysCategory.getCode(), sysCategory.getPid());
            if (category != null && !category.getId().equals(sysCategory.getId())
                    && category.getPid().equals(sysCategory.getPid())) {
                return "编码已存在";
            }
        }
